...
void PresidentialPardonForm::execute(Bureaucrat const & executor) const
{
	if (!_signed)	
		...
		throw PresidentialPardonForm::gradeToLowException(); 
	else if (executor.getGrade() < 1)
		throw PresidentialPardonForm::gradeToHighException(); 
	else
		cout << BOLD << _target + " has been pardoned by Zaphod Beeblebrox\n"
		<< END; 
